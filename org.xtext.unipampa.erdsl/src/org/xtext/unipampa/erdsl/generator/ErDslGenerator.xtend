...
class ErDslGenerator extends AbstractGenerator {
	...
	«FOR entity : e.entities SEPARATOR "\n);\n" AFTER ");\n"»
		...
		CREATE TABLE IF NOT EXISTS «entity.name.toLowerCase» (
			...
			«attTypeCheckerPG(auxAttributes)»
			«ENDIF»«ENDFOR»«ENDIF»«ENDFOR»«ENDIF»
			«FOR attribute : entity.attributes»«attTypeCheckerPG(attribute)»«ENDFOR»
			«IF !(entity.is === null)»«FOR aux : e.entities»«IF aux.name.equalsIgnoreCase(entity.is.toString)»«FOR auxAttributes : aux.attributes»«IF auxAttributes.isIsKey»
			...
			«verifyFKsAttributesRelation1to1(e, entity.name)»
			«verifyFKsAttributesRelation1toN(e, entity.name)»
			PRIMARY KEY («FOR x : myListPKs SEPARATOR ", " AFTER ")"»«println(x.toLowerCase)»«ENDFOR»«IF !myListExtendPKs.nullOrEmpty»,«ENDIF»
			«FOR x : myListExtendPKs SEPARATOR ",\n"»FOREIGN KEY («println(x.toString.toLowerCase)») REFERENCES «discoverInheritedPKtoFK(e,x.toString)»«ENDFOR»
			«myListPKs.clear»«myListExtendPKs.clear»
			«defineFKsRelation1to1(e, entity.name)»
			«defineFKsRelation1toN(e, entity.name)»
	«ENDFOR»
	
	/* RELAÇÕES N PARA N */ ««« Relaçẽos n para n »»»
	«FOR relation : e.relations»
		...
		(relation.rightEnding.cardinality.equalsIgnoreCase('(0:N)') || relation.rightEnding.cardinality.equalsIgnoreCase('(1:N)')))»		
			...
			«IF relation.leftEnding.target.toString.equalsIgnoreCase(entity.name) && (relation.leftEnding.target.toString !== relation.rightEnding.target.toString)»«{myListPKsFKs.add(relation.leftEnding.target.toString);""}»	«relation.leftEnding.target.toString.toLowerCase»«attTypeCheckerNtoNLeft(e,relation)»«ENDIF»
			«IF relation.rightEnding.target.toString.equalsIgnoreCase(entity.name) && (relation.rightEnding.target.toString !== relation.leftEnding.target.toString)»«{myListPKsFKs.add(relation.rightEnding.target.toString);""}»	«relation.rightEnding.target.toString.toLowerCase»«attTypeCheckerNtoNRight(e,relation)»«ENDIF»
«««					«/**
«««					*
...
«««					«ENDIF»				
			«ENDFOR»	
				«IF !relation.attributes.nullOrEmpty»«FOR aux : relation.attributes»«attTypeCheckerPG(aux)»«ENDFOR»
			«ENDIF»
				...
				«FOR x : myListPKsFKs SEPARATOR "),"»FOREIGN KEY («println(x.toString.toLowerCase)») REFERENCES «print(x.toString.toLowerCase)» («discoverPKtoFK(e,x)»«ENDFOR»)«myListPKsFKs.clear»
			);
			«ENDIF»
	«ENDFOR»
	'''
	
	private def attTypeCheckerPG(Attribute a) '''
		«a.name.toLowerCase» «IF a.type.toString.equalsIgnoreCase("string")» VARCHAR (255) NOT NULL,
				...
				«ENDIF»
	'''
	private def attTypeCheckerNtoNLeft (ERModel e, Relation r)'''
		«FOR aux : e.entities»
			«IF aux.name.toString == r.leftEnding.target.toString»
				«FOR aux2 : aux.attributes»
					«IF aux2.isIsKey»
						...
						«ELSEIF aux2.type.toString.equalsIgnoreCase("money") || aux2.type.toString.equalsIgnoreCase("double")» NUMERIC NOT NULL,
	...
	private def attTypeCheckerNtoNRight (ERModel e, Relation r)'''
		«FOR aux : e.entities»
	...
	private def discoverPKtoFK (ERModel e, String r)'''
		«FOR aux : e.entities»
			«IF aux.name.toString == r.toString»
				«FOR aux2 : aux.attributes»
					«IF aux2.isIsKey»«aux2.name.toString.toLowerCase»«ENDIF»
				«ENDFOR»
			«ENDIF»
		«ENDFOR»
	'''
	
	private def discoverInheritedPKtoFK (ERModel e, String r)'''
		«FOR aux : e.entities»
			«FOR aux2 : aux.attributes»
				«IF aux2.name.toString.equalsIgnoreCase(r.toString) && aux2.isIsKey»
					«aux.name.toString.toLowerCase» («aux2.name.toString.toLowerCase»)
				«ENDIF»
			«ENDFOR»
		«ENDFOR»
	'''
	
	private def verifyFKsAttributesRelation1to1 (ERModel e, String ename) '''
		«FOR relation : e.relations»
			«IF ((relation.leftEnding.cardinality.equalsIgnoreCase('(0:1)') || relation.leftEnding.cardinality.equalsIgnoreCase('(1:1)'))
		&& 
		(relation.rightEnding.cardinality.equalsIgnoreCase('(0:1)') || relation.rightEnding.cardinality.equalsIgnoreCase('(1:1)')))»
	...
	private def verifyFKsAttributesRelation1toN (ERModel e, String ename) '''
		«FOR relation : e.relations»
			«IF (((relation.leftEnding.cardinality.equalsIgnoreCase('(0:1)') || relation.leftEnding.cardinality.equalsIgnoreCase('(1:1)'))
					&& 
					(relation.rightEnding.cardinality.equalsIgnoreCase('(0:N)') || relation.rightEnding.cardinality.equalsIgnoreCase('(1:N)')))) 
						...
						«IF relation.leftEnding.target.toString.equalsIgnoreCase(aux.name)»
							«IF aux.is === null»
								«FOR attTest : aux.attributes»
									«IF attTest.isIsKey»
									««« Casos FK3»»»
										!!! FK3 !!! «attTypeCheckerPG(attTest)» 
									«ENDIF»
								«ENDFOR»
							«ELSEIF !(aux.is === null)»
								«IF relation.leftEnding.target.toString.equalsIgnoreCase(relation.rightEnding.target.toString)»
								««« Casos FK4»»»
									!!! FK4 !!! «relation.name.toString»,
								«ELSEIF !relation.leftEnding.target.toString.equalsIgnoreCase(relation.rightEnding.target.toString)»
								««« Casos FK5»»»
									!!! FK5 !!! «relation.leftEnding.target.toString»,
								«ENDIF»
							«ENDIF»
						«ENDIF»
					«ENDFOR»
	...
	private def defineFKsRelation1to1(ERModel e, String ename) '''
		«FOR relation : e.relations»
			«IF ((relation.leftEnding.cardinality.equalsIgnoreCase('(0:1)') || relation.leftEnding.cardinality.equalsIgnoreCase('(1:1)'))
			&& 
			(relation.rightEnding.cardinality.equalsIgnoreCase('(0:1)') || relation.rightEnding.cardinality.equalsIgnoreCase('(1:1)')))»
						...
						«IF relation.leftEnding.target.toString.equalsIgnoreCase(aux.name)»
							«IF aux.is === null»
								«FOR aux2 : aux.attributes»
									«IF aux2.isIsKey»
										««« Casos FK1»»»
											FOREIGN KEY («aux2.name.toString.toLowerCase») REFERENCES «aux.name.toString» («aux2.name.toString.toLowerCase»)
									«ENDIF»
								«ENDFOR»
							«ELSEIF !(aux.is === null)»
								FOREIGN KEY ( ) REFERENCES ( )«aux.is.toString» !!! FK2 !!!
							«ENDIF»
						«ENDIF»
					«ENDFOR»
				«ENDIF»
	...
	private def defineFKsRelation1toN(ERModel e, String ename) '''
		«FOR relation : e.relations»
			«IF (((relation.leftEnding.cardinality.equalsIgnoreCase('(0:1)') || relation.leftEnding.cardinality.equalsIgnoreCase('(1:1)'))
						&& 
						(relation.rightEnding.cardinality.equalsIgnoreCase('(0:N)') || relation.rightEnding.cardinality.equalsIgnoreCase('(1:N)')))) 
						...
						«IF relation.leftEnding.target.toString.equalsIgnoreCase(aux.name)»
							«IF aux.is === null»
								«FOR attTest : aux.attributes»
									«IF attTest.isIsKey»
										««« Casos FK3»»»
											FOREIGN KEY («attTest.name.toString.toLowerCase») REFERENCES «aux.name.toString» («attTest.name.toString.toLowerCase»)
									«ENDIF»
								«ENDFOR»
							«ELSEIF !(aux.is === null)»
								«IF relation.leftEnding.target.toString.equalsIgnoreCase(relation.rightEnding.target.toString)»
									FOREIGN KEY («relation.name.toString») REFERENCES «relation.leftEnding.target.toString.toLowerCase» ( ) !!! FK4 !!!
								«ELSEIF !relation.leftEnding.target.toString.equalsIgnoreCase(relation.rightEnding.target.toString)»
									FOREIGN KEY ( ) REFERENCES («relation.leftEnding.target.toString») !!! FK5 !!!
								«ENDIF»
							«ENDIF»
						«ENDIF»
					«ENDFOR»
	...
	private def attributeIdentifierCheckerPG(Attribute a) '''
