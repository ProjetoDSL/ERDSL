...
class ErDslGenerator extends AbstractGenerator {
	...
	override void doGenerate(Resource resource, IFileSystemAccess2 fsa, IGeneratorContext context) {
		...
		for (diagramModel : resource.contents.filter(typeof(ERModel))) {
			...
			if (fsa instanceof IFileSystemAccessExtension3) {
				...
				fsa.generateFile(modeloER.domain.name + "_diagramDescriptorGenerated.txt", plantUML)
			} else {
				//fsa.generateFile(filename + "_diagram.puml", plantUML)
				fsa.generateFile(modeloER.domain.name + "_diagramParcialDescriptorGenerated.txt", plantUML)
			}
		}
				
	}
	
	def dispatch CharSequence plotToPlantUML(ERModel it) '''
    @startuml
...
«««    «ENDFOR»
    ...
    def dispatch CharSequence plotToPlantUML(Entity it) '''
...
//	One or Many		|	}|--
    ...
    def dispatch CharSequence plotToPlantUML(Relation it) ''' 
...
//	def defineLeftCardinalitySymbolUML(String cd)'''«IF cd.equalsIgnoreCase("(0:1)")»"(0:1)" «ELSEIF cd.equalsIgnoreCase("(1:1)")»"(1:1)" «ELSEIF cd.equalsIgnoreCase("(0:N)")»"(0:N)" «ELSEIF cd.equalsIgnoreCase("(1:N)")»"(1:N)" «ENDIF»'''
...
//	def defineRightCardinalitySymbolUML(String cd)'''«IF cd.equalsIgnoreCase("(0:1)")» "(0:1)"«ELSEIF cd.equalsIgnoreCase("(1:1)")» "(1:1)" «ELSEIF cd.equalsIgnoreCase("(0:N)")» "(0:N)" «ELSEIF cd.equalsIgnoreCase("(1:N)")» "(1:N)" «ENDIF»'''
